...
//     useEffect(()=>{
//     async function Data(){
//         fetch(`http://beet.asia:9200/doublec_pytorch/_search`,{method:'Get'}).then((response)=>{console.log(response)});
//     }
//     Data()
// },[])
...
for (var n = 0; n < days_pie.length; n++) {
    ...
    var res = [];
    for (var j = 0; j < data_pie[n].length; j++) {
        res.push({
            name: category_pie[j],
            value: data_pie[n][j]
        });
  ...
  return (
    <Card>
      ...
      <ReactEchars option={option}></ReactEchars>
      </Box>
    </Card>
  );
};
